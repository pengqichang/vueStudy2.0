(function (global, factory) {
  ...
  function patch(oldVnode, vnode) {
    ...
    if (!oldVnode) {
      ...
      if (isRealElement) {
        ...
        updateProperties(vnode, oldVnode.data); // 需要比对子节点
        ...
        if (oldChildren.length > 0 && newChildren.length) {
          ...
          for (var i in newChildren) {
            var child = newChildren[i]; // 此时是虚拟节点
  ...
  function updateChildren(parent, oldChildren, newChildren) {
    ...
    while (oldStartIndex <= oldEndIndex && newStartIndex <= newEndIndex) {
      // 优化向后插入的情况
      if (!oldStartVnode) {
        // 在老指针移动的过程中，可能会碰到 undefined 的情况
      ...
      if (isSameVnode(oldStartVnode, newStartVnode)) {
        ...
        oldEndVnode = oldChildren[--oldEndIndex];
        newEndVnode = newChildren[--newEndIndex]; // 头移尾  （涉及到 倒序变成正序）
      } else if (isSameVnode(oldStartVnode, newEndVnode)) {
        patch(oldStartVnode, newEndVnode);
        parent.insertBefore(oldStartVnode.el, oldEndVnode.el.nextSibling);
        oldStartVnode = oldChildren[++oldStartIndex];
        newEndVnode = newChildren[--newEndIndex]; // 尾移头   （涉及到正序变成倒序）
        ...
        if (!moveIndex) {
          ...
          var moveVnode = oldChildren[moveIndex]; // 查找到对应的 key，开始移动
  ...
  function updateProperties(vnode) {
    ...
    var el = vnode.el; // 如果老的属性中有，新的属性中没有，要再真是的 dom 中删除此属性
  ...
  initGlobalAPI(Vue); // demo 产生两个虚拟节点进行比对    dom-diff
  var vm1 = new Vue({
    data: {
      name: 'hello'
    }
  });
  var render = complieToFunction("<div id=aap >\n  <div>A</div>\n  <div>B</div>\n  <div>C</div>\n  <div>D</div>\n</div>");
  var vnode$1 = render.call(vm1);
  ...
  var render1 = complieToFunction("<div id=app1>\n<p>E</p>\n  <div>A</div>\n  <div>B</div>\n  <div>C</div>\n  <div>D</div>\n</div>");
  var newVnode = render1.call(vm2); // let el1 = createElm(newVnode)
  // console.log(vnode, newVnode)

  patch(vnode$1, newVnode); // 如果传入两个虚拟节点后，会再内部进行比对
